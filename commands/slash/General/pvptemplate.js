...
module.exports = {
  ...
  run: async (client, interaction, config) => {
    ...
    if (
      !member.roles.cache.has("1068815986121244692") &&
      member.id !== config.Users.OWNERS
    ) {
      return interaction.reply(
        "Only Content Creator and the owner can execute this command."
      );
    }
    ...
    const emojiarray = [
      maintankemoji,
      frontlinedeftankemoji,
      backlinedeftankemoji,
      clumptankemoji,
      mainhealeremoji,
      blighthealeremoji,
      onehandnatureemoji,
...
module.exports.fetchPvPReactions = async function (message) {
  try {
    ...
    if (reactionCollection.has(frontlinedeftankemoji)) {
      const frontlinedeftankReaction = reactionCollection.get(
        frontlinedeftankemoji
      );
      const frontlinedeftankUserIds = (
        await frontlinedeftankReaction.users.fetch()
      )
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(backlinedeftankemoji)) {
      const backlinedeftankReaction =
        reactionCollection.get(backlinedeftankemoji);
      const backlinedeftankUserIds = (
        await backlinedeftankReaction.users.fetch()
      )
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(clumptankemoji)) {
      ...
      const clumptankUserIds = (await clumptankReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(blighthealeremoji)) {
      ...
      const blighthealerUserIds = (await blighthealerReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(onehandnatureemoji)) {
      ...
      const onehandnatureUserIds = (await onehandnatureReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(greatholyemoji)) {
      ...
      const greatholyUserIds = (await greatholyReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(enigmaticemoji)) {
      ...
      const enigmaticUserIds = (await enigmaticReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(lifecurseemoji)) {
      ...
      const lifecurseUserIds = (await lifecurseReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(bloodletteremoji)) {
      ...
      const bloodletterUserIds = (await bloodletterReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(fistsofavalonemoji)) {
      ...
      const fistsofavalonUserIds = (await fistsofavalonReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(spikedgauntletsemoji)) {
      const spikedgauntletsReaction =
        reactionCollection.get(spikedgauntletsemoji);
      const spikedgauntletsUserIds = (
        await spikedgauntletsReaction.users.fetch()
      )
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(demonfangemoji)) {
      ...
      const demonfangUserIds = (await demonfangReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(permafrostemoji)) {
      ...
      const permafrostUserIds = (await permafrostReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(dawnsongemoji)) {
      ...
      const dawnsongUserIds = (await dawnsongReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
    ...
    if (reactionCollection.has(siegebowemoji)) {
      ...
      const siegebowUserIds = (await siegebowReaction.users.fetch())
        .filter((user) => !user.bot)
        .map((user) => user.id);
          ...
          .addFields([
            {
              ...
              value: `<:total:1129625110605737994> ** ${
                ...
                frontlinedeftankusers.length +
                backlinedeftankusers.length +
                clumptankusers.length +
                mainhealerusers.length +
                blighthealerusers.length +
                onehandnatureusers.length +
                greatholyusers.length +
                enigmaticusers.length +
                lifecurseusers.length +
                bloodletterusers.length +
                carvingusers.length +
                realmbreakerusers.length +
                fistsofavalonusers.length +
                spikedgauntletsusers.length +
                spikedgauntletsusers.length +
                demonfangusers.length +
                permafrostusers.length +
                dawnsongusers.length +
                siegebowusers.length
              } **`,
              inline: false,
            },
            {
            ...
            },
            {
              ...
              value: ` ${maintankusers
                .map((user) => "<:1HMace:1139482862937178172> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },

            {
              ...
              value: ` ${frontlinedeftankusers
                .map(
                  (user) => "<:GroveKeeper:1139482983397597196> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${backlinedeftankusers
                .map((user) => "<:HeavyMace:1139483020533960764> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:HandofJustice:1139483008668270662> __**ClumpTank**__(${clumptankusers.length})`,
              value: ` ${clumptankusers
                .map(
                  (user) => "<:HandofJustice:1139483008668270662> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${mainhealerusers
                .map((user) => "<:HallowFall:1139482997255569439> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:Blight:1139482874257621052> __**BlightHeal**__(${blighthealerusers.length})`,
              value: ` ${blighthealerusers
                .map((user) => "<:Blight:1139482874257621052> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:1HandNature:1139482850228453437> __**1HNatureHeal**__(${onehandnatureusers.length})`,
              value: ` ${onehandnatureusers
                .map(
                  (user) => "<:1HandNature:1139482850228453437> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${greatholyusers
                .map((user) => "<:GreatHoly:1139482975755587665> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:Enigmatic:1139482935246995487> __**Enigmatic**__(${enigmaticusers.length})`,
              value: ` ${enigmaticusers
                .map((user) => "<:Enigmatic:1139482935246995487> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:Lifecurse:1139483040750510152> __**Lifecurse**__(${lifecurseusers.length})`,
              value: ` ${lifecurseusers
                .map((user) => "<:Lifecurse:1139483040750510152> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:Bloodletter:1139482885787766804> __**Bloodletter**__(${bloodletterusers.length})`,
              value: ` ${bloodletterusers
                .map(
                  (user) => "<:Bloodletter:1139482885787766804> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${carvingusers
                .map((user) => "<:Carving:1139482896369987585> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${realmbreakerusers
                .map(
                  (user) => "<:Realmbreaker:1139482815679975444> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${fistsofavalonusers
                .map(
                  (user) => "<:FistsofAvalon:1139482963491438674> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${spirithunterusers
                .map(
                  (user) => "<:Spirithunter:1139482844058615838> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${spikedgauntletsusers
                .map(
                  (user) =>
                    "<:SpikedGauntlets:1139482831123390584> " + user.name
                )
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${demonfangusers
                .map((user) => "<:Demonfang:1139482924203380756> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              name: `<:Permafrost:1139483049550151780> __**Permafrost**__(${permafrostusers.length})`,
              value: ` ${permafrostusers
                .map((user) => "<:Permafrost:1139483049550151780> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${dawnsongusers
                .map((user) => "<:Dawnsong:1139482904079114272> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
              ...
              value: ` ${siegebowusers
                .map((user) => "<:Siegebow:1139482824064368691> " + user.name)
                .join("\n")} \n\u200B`,
              inline: true,
            },
            {
